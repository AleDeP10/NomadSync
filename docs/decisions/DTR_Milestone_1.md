...
## [M1] `SyncOrchestrator` as intermediate layer between `io.obsidiansync.io.obsidiansync.Main` and `GitService`

**Context**: the logic for coordinating operations (e.g. stash before pull, exit code
handling) belongs neither to `io.obsidiansync.io.obsidiansync.Main` nor to `GitService`.

**Decision**: introduce `SyncOrchestrator` as a dedicated layer. `io.obsidiansync.io.obsidiansync.Main` calls the
